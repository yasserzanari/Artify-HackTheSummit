...
export function useArtworkQA(options: UseArtworkQAOptions): UseArtworkQAReturn {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true })
      ...
      .catch((err) => {
        if (err instanceof DOMException && err.name === 'NotAllowedError') {
          audioLogger.log('WARN', 'QA', 'mic permission denied — Q&A disabled for this session')
  ...
  const startQA = useCallback(async () => {
    ...
    while (activeRef.current) {
      ...
      // Wait for visitor to tap "Done" or 30s auto-stop
    ...
    qaAvailable, artwork, narrationHistory, ttsService, sttService, geminiService,
